...
public class RecordFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        if (cid != -1) {
            //Edit
            App.getInstance().getDataBase().contactDao().findById(cid).observe(getViewLifecycleOwner(), contact -> {
        ...
        register.setOnClickListener(view1 -> {
            ...
            new Thread(() -> {
                if (cid != -1) {
                    contact.cid = cid;
                    App.getInstance().getDataBase().contactDao().updateAll(contact);
                } else {
                    App.getInstance().getDataBase().contactDao().insertAll(contact);
                }
                ...
                getActivity().runOnUiThread(() -> {
                    Snackbar.make(getActivity().findViewById(android.R.id.content), cid != -1 ? "Contact updated" : "Contact created", Snackbar.LENGTH_INDEFINITE)
                            .setAction("Ok", view2 -> NavHostFragment.findNavController(RecordFragment.this).navigateUp())
